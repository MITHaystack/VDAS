...
PFR::PFR(const char* _device_name, u_int _snaplen, bool promisc, char *bpf) {
...
int PFR::add_bpf_filter(char *the_filter) {
...
int PFR::get_next_packet(struct pfring_pkthdr *hdr, const u_char *pkt, u_int pkt_len) {
...
bool PFR::wait_for_packets(int msec) {
  struct pollfd pfd;
  int rc;
